...
class Serializer:
    ...
    def dumps(
        ...
        if body is None:
            ...
            response._fp = io.BytesIO(body)  # types: ignore[attr-defined]
            response.length_remaining = len(body)

        data = {
            "response": {
                "body": body,  # Empty bytestring if body is stored separately
                "headers": {str(k): str(v) for k, v in response.headers.items()},  # types: ignore[no-untyped-call]
    ...
    def loads(
        ...
        try:
            return getattr(self, f"_loads_v{verstr}")(request, data, body_file)  # types: ignore[no-any-return]
