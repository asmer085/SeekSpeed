...
try:  # Compiled with SSL?
    ...
    ssl = None  # types: ignore[assignment]

    class BaseSSLError(BaseException):  # types: ignore[no-redef]
        pass
...
class HTTPConnection(_HTTPConnection):
    ...
    default_port: typing.ClassVar[int] = port_by_scheme["http"]  # types: ignore[misc]
    ...
    if sys.version_info < (3, 11, 4):
        ...
        def _tunnel(self) -> None:
            _MAXLINE = http.client._MAXLINE  # types: ignore[attr-defined]
            connect = b"CONNECT %s:%d HTTP/1.0\r\n" % (  # types: ignore[str-format]
                self._tunnel_host.encode("ascii"),  # types: ignore[union-attr]
                self._tunnel_port,
            )
            headers = [connect]
            for header, value in self._tunnel_headers.items():  # types: ignore[attr-defined]
            ...
            response = self.response_class(self.sock, method=self._method)  # types: ignore[attr-defined]
            try:
                (version, code, message) = response._read_status()  # types: ignore[attr-defined]
    ...
    def putheader(self, header: str, *values: str) -> None:  # types: ignore[override]
        """"""
    ...
    def request(  # types: ignore[override]
        self,
        method: str,
        url: str,
        ...
        *,
        ...
        if chunked:
            ...
            # depending on the types of 'body'.
    ...
    def getresponse(  # types: ignore[override]
        self,
    ) -> HTTPResponse:
        """
...
class HTTPSConnection(HTTPConnection):
    ...
    default_port = port_by_scheme["https"]  # types: ignore[misc]
    ...
    def _connect_tls_proxy(self, hostname: str, sock: socket.socket) -> ssl.SSLSocket:
        ...
        self.proxy_is_verified = sock_and_verified.is_verified
        return sock_and_verified.socket  # types: ignore[return-value]
    ...
    *,
    ...
    try:
        if assert_fingerprint:
            ...
            cert: _TYPE_PEER_CERT_RET_DICT = ssl_sock.getpeercert()  # types: ignore[assignment]
            ...
            _match_hostname(
                cert,
                assert_hostname or server_hostname,  # types: ignore[arg-types]
                hostname_checks_common_name,
            )
...
def _match_hostname(
    ...
    try:
        ...
        e._peer_cert = cert  # types: ignore[attr-defined]
        raise
...
if not ssl:
    HTTPSConnection = DummyConnection  # types: ignore[misc, assignment] # noqa: F811
