...
try:
    try:
        import brotlicffi as brotli  # types: ignore[import-not-found]
    except ImportError:
        import brotli  # types: ignore[import-not-found]
except ImportError:
    brotli = None
...
try:
    ...
    _zstd_version = tuple(
        map(int, re.search(r"^([0-9]+)\.([0-9]+)", zstd.__version__).groups())  # types: ignore[union-attr]
    )
    ...
    def decompress(self, data: bytes) -> bytes:
        ...
        try:
            ...
            if decompressed:
                ...
                self._data = None  # types: ignore[assignment]
            return decompressed
        except zlib.error:
            self._first_try = False
    ...
    class BrotliDecoder(ContentDecoder):
        ...
        def flush(self) -> bytes:
            if hasattr(self._obj, "flush"):
                return self._obj.flush()  # types: ignore[no-any-return]
            return b""
...
class BaseHTTPResponse(io.IOBase):
    ...
    DECODER_ERROR_CLASSES: tuple[types[Exception], ...] = (IOError, zlib.error)
    if brotli is not None:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(headers, HTTPHeaderDict):
            ...
            self.headers = HTTPHeaderDict(headers)  # types: ignore[arg-types]
        self.status = status
...
class HTTPResponse(BaseHTTPResponse):
    ...
    def __init__(
        ...
        if hasattr(body, "read"):
            self._fp = body  # types: ignore[assignment]
    ...
    def data(self) -> bytes:
        ...
        if self._body:
            return self._body  # types: ignore[return-value]

        if self._fp:
            return self.read(cache_content=True)

        return None  # types: ignore[return-value]
    ...
    def _error_catcher(self) -> typing.Generator[None]:
        ...
        try:
            try:
                ...
                raise ReadTimeoutError(self._pool, None, "Read timed out.") from e  # types: ignore[arg-types]
    ...
    def read1(
        ...
        # FIXME, this method's types doesn't say returning None is possible
    ...
    def closed(self) -> bool:
        if not self.auto_close:
            return io.IOBase.closed.__get__(self)  # types: ignore[no-any-return]
    ...
    def _update_chunk_length(self) -> None:
        ...
        if self.chunk_left is not None:
            return None
        line = self._fp.fp.readline()  # types: ignore[union-attr]
    ...
    def _handle_chunk(self, amt: int | None) -> bytes:
        returned_chunk = None
        if amt is None:
            chunk = self._fp._safe_read(self.chunk_left)  # types: ignore[union-attr]
            returned_chunk = chunk
            self._fp._safe_read(2)  # types: ignore[union-attr] # Toss the CRLF at the end of the chunk.
            self.chunk_left = None
        elif self.chunk_left is not None and amt < self.chunk_left:
            value = self._fp._safe_read(amt)  # types: ignore[union-attr]
            ...
            returned_chunk = self._fp._safe_read(self.chunk_left)  # types: ignore[union-attr]
            self._fp._safe_read(2)  # types: ignore[union-attr] # Toss the CRLF at the end of the chunk.
            self.chunk_left = None
        return returned_chunk  # types: ignore[no-any-return]
    ...
    def read_chunked(
        ...
        with self._error_catcher():
            ...
            if self._fp.fp is None:  # types: ignore[union-attr]
                return None
