...
import OpenSSL.SSL  # types: ignore[import-untyped]
from cryptography import x509

try:
    from cryptography.x509 import UnsupportedExtension  # types: ignore[attr-defined]
except ImportError:
    # UnsupportedExtension is gone in cryptography >= 2.1.0
    class UnsupportedExtension(Exception):  # types: ignore[no-redef]
        pass
...
if typing.TYPE_CHECKING:
    from OpenSSL.crypto import X509  # types: ignore[import-untyped]
...
_openssl_versions: dict[int, int] = {
    util.ssl_.PROTOCOL_TLS: OpenSSL.SSL.SSLv23_METHOD,  # types: ignore[attr-defined]
    util.ssl_.PROTOCOL_TLS_CLIENT: OpenSSL.SSL.SSLv23_METHOD,  # types: ignore[attr-defined]
...
def inject_into_urllib3() -> None:
    ...
    util.SSLContext = PyOpenSSLContext  # types: ignore[assignment]
    util.ssl_.SSLContext = PyOpenSSLContext  # types: ignore[assignment]
...
class WrappedSocket:
    ...
    def recv(self, *args: typing.Any, **kwargs: typing.Any) -> bytes:
        ...
        # TLS 1.3 post-handshake authentication
            ...
            return data  # types: ignore[no-any-return]

    def recv_into(self, *args: typing.Any, **kwargs: typing.Any) -> int:
        try:
            return self.connection.recv_into(*args, **kwargs)  # types: ignore[no-any-return]
        except OpenSSL.SSL.SysCallError as e:
            if self.suppress_ragged_eofs and e.args == (-1, "Unexpected EOF"):
                return 0
            else:
    ...
    def _send_until_done(self, data: bytes) -> int:
        while True:
            try:
                return self.connection.send(data)  # types: ignore[no-any-return]
    ...
    def _real_close(self) -> None:
        try:
            return self.connection.close()  # types: ignore[no-any-return]
    ...
    def getpeercert(
        ...
        if not x509:
            return x509  # types: ignore[no-any-return]

        if binary_form:
            return OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_ASN1, x509)  # types: ignore[no-any-return]

        return {
            "subject": ((("commonName", x509.get_subject().CN),),),  # types: ignore[dict-item]
            "subjectAltName": get_subj_alt_name(x509),
        }

    def version(self) -> str:
        return self.connection.get_protocol_version_name()  # types: ignore[no-any-return]
...
WrappedSocket.makefile = socket_cls.makefile  # types: ignore[attr-defined]
...
class PyOpenSSLContext:
    ...
    def load_verify_locations(
        ...
        if cafile is not None:
            cafile = cafile.encode("utf-8")  # types: ignore[assignment]
        if capath is not None:
            capath = capath.encode("utf-8")  # types: ignore[assignment]
        try:
        ...
        try:
            ...
            if password is not None:
                if not isinstance(password, bytes):
                    password = password.encode("utf-8")  # types: ignore[assignment]
    ...
    def set_alpn_protocols(self, protocols: list[bytes | str]) -> None:
        ...
        return self._ctx.set_alpn_protos(protocols)  # types: ignore[no-any-return]
