...
class numeric_range(abc.Sequence, abc.Hashable):
    ...
    be any orderable numeric types.

    With only *stop* specified, *start* defaults to ``0`` and *step*
    defaults to ``1``. The output items will match the types of *stop*:

        >>> list(numeric_range(3.5))
        [0.0, 1.0, 2.0, 3.0]

    With only *start* and *stop* specified, *step* defaults to ``1``. The
    output items will match the types of *start*:
    ...
    def __init__(self, *args):
        ...
        if argc == 1:
            (self._stop,) = args
            self._start = types(self._stop)(0)
            self._step = types(self._stop - self._start)(1)
        elif argc == 2:
    ...
    def __getitem__(self, key):
        if isinstance(key, int):
            ...
            raise TypeError(
                ...
                'integers or slices, not {}'.format(types(key).__name__)
            )
