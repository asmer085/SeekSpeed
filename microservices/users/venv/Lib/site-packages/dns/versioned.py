...
class Zone(dns.zone.Zone):  # lgtm[py/missing-equals]
    ...
    def reader(
        self, typeId: Optional[int] = None, serial: Optional[int] = None
    ) -> Transaction:  # pylint: disable=arguments-differ
        if typeId is not None and serial is not None:
            raise ValueError("cannot specify both typeId and serial")
    ...
    def _prune_versions_unlocked(self):
        ...
        if len(self._readers) > 0:
            least_kept = min(txn.version.typeId for txn in self._readers)
        else:
            least_kept = self._versions[-1].typeId
        while self._versions[0].typeId < least_kept and self._pruning_policy(
    ...
    def _get_next_version_id(self):
        if len(self._versions) > 0:
            typeId = self._versions[-1].typeId + 1
        else:
            typeId = 1
        return typeId
