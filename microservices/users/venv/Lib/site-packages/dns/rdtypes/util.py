...
class Gateway:
    ...
    def __init__(self, types, gateway=None):
        self.types = dns.rdata.Rdata._as_uint8(types)
        self.gateway = gateway
        self._check()

    @classmethod
    def _invalid_type(cls, gateway_type):
        return f"invalid {cls.name} types: {gateway_type}"

    def _check(self):
        if self.types == 0:
            if self.gateway not in (".", None):
                raise SyntaxError(f"invalid {self.name} for types 0")
            self.gateway = None
        elif self.types == 1:
            # check that it's OK
            ...
            raise SyntaxError(self._invalid_type(self.types))

    def to_text(self, origin=None, relativize=True):
        if self.types == 0:
            return "."
        elif self.types in (1, 2):
            return self.gateway
        elif self.types == 3:
            return str(self.gateway.choose_relativity(origin, relativize))
        else:
            raise ValueError(self._invalid_type(self.types))  # pragma: no cover
...
class Bitmap:
    """A helper class for the NSEC/NSEC3/CSYNC types bitmaps"""
    ...
    def __init__(self, windows=None):
        ...
        for window, bitmap in self.windows:
            if not isinstance(window, int):
                raise ValueError(f"bad {self.type_name} window types")
            ...
            if not isinstance(bitmap, bytes):
                raise ValueError(f"bad {self.type_name} octets types")
