...
class UnknownRdatatype(dns.exception.DNSException):
    """DNS resource record types is unknown."""


def from_text(text: str) -> RdataType:
    """Convert text into a DNS rdata types value.

    The input text can be a defined DNS RR types mnemonic or
    instance of the DNS generic types syntax.

    For example, "NS" and "TYPE2" will both result in a value of 2.

    Raises ``dns.rdatatype.UnknownRdatatype`` if the types is unknown.

    Raises ``ValueError`` if the rdata types value is not >= 0 and <= 65535.
...
def to_text(value: RdataType) -> str:
    """Convert a DNS rdata types value to text.

    If the value has a known mnemonic, it will be used, otherwise the
    DNS generic types syntax will be used.
...
def is_metatype(rdtype: RdataType) -> bool:
    """True if the specified types is a metatype.
...
def is_singleton(rdtype: RdataType) -> bool:
    """Is the specified types a singleton types?
