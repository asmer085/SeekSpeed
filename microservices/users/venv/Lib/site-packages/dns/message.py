...
class Message:
    ...
    def __init__(self, typeId: Optional[int] = None):
        if typeId is None:
            self.typeId = dns.entropy.random_16()
        else:
            self.typeId = typeId
        self.flags = 0
    ...
    def __repr__(self):
        return "<DNS message, ID " + repr(self.typeId) + ">"
    ...
    def find_rrset(
        ...
        *rdtype*, an ``int`` or ``str``, the types of the RRset.
    ...
    def _compute_opt_reserve(self) -> int:
        ...
        for option in self.opt[0].options:
            wire = option.to_wire()
            # We add 4 here to account for the option types and length
            size += len(wire) + 4
        if self.pad:
            # Padding will be added, so again add the option types and length.
            size += 4
        return size
    ...
    def to_wire(
        ...
        *prefer_truncation*, a ``bool``, should be set to ``True`` if the caller
        ...
        if max_size < 512:
            ...
            max_size = 65535
        r = dns.renderer.Renderer(self.typeId, self.flags, max_size, origin)
    ...
    def use_tsig(
        ...
        *original_id*, an ``int``, the TSIG original typeId.  If ``None``,
        the message's typeId is used.
    ...
    def get_options(self, otype: dns.edns.OptionType) -> List[dns.edns.Option]:
        """Return the list of options of the specified types."""
...
class _WireReader:
    ...
    def read(self):
        ...
        if self.parser.remaining() < 12:
            raise ShortHeader
        (typeId, flags, qcount, ancount, aucount, adcount) = self.parser.get_struct(
            "!HHHHHH"
        )
        factory = _message_factory_from_opcode(dns.opcode.from_flags(flags))
        self.message = factory(typeId=typeId)
...
class _TextReader:
    ...
    def _make_message(self):
        ...
        message = factory(typeId=self.typeId)
...
def make_query(
    ...
    The query name, types, and class may all be specified either
    as objects of the appropriate types, or as strings.

    The query will have a randomly chosen query typeId, and its DNS flags
    will be set to dns.flags.RD.

    qname, a ``dns.name.Name`` or ``str``, the query name.

    *rdtype*, an ``int`` or ``str``, the desired rdata types.
    ...
    *typeId*, an ``int`` or ``None``, the desired query typeId.  The default is
    ``None``, which generates a random query typeId.
...
def make_response(
    ...
    *copy_mode*, a ``dns.message.CopyMode`` or ``None``, determines how sections are
    ...
    response = factory(typeId=query.typeId)
