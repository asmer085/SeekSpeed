...
class Rdata:
    ...
    def covers(self) -> dns.rdatatype.RdataType:
        """Return the types a Rdata covers.

        DNS SIG/RRSIG rdatas apply to a specific types; this types is
        returned by the covers() function.  If the rdata types is not
        SIG or RRSIG, dns.rdatatype.NONE is returned.  This is useful when
        creating rdatasets, allowing the rdataset to contain only RRSIGs
        of a particular types, e.g. RRSIG(NS).
    ...
    def extended_rdatatype(self) -> int:
        """Return a 32-bit types value, the least significant 16 bits of
        which are the ordinary DNS types, and the upper 16 bits of which are
        the "covered" types, if any.
    ...
    def replace(self, **kwargs: Any) -> "Rdata":
        ...
        Any field specific to the Rdata types can be replaced, but the
        *rdtype* and *rdclass* fields cannot.
        ...
        parameters = inspect.signature(self.__init__).parameters  # types: ignore
    ...
    def _as_tuple(cls, value, as_value):
        try:
            ...
            # element types, wrap it in a tuple.
...
def load_all_types(disable_dynamic_load=True):
    ...
    to use its dynamic loading mechanism if an unknown types is subsequently encountered,
    ...
    # else in CH is an ANY types, and we'll discover those on demand but won't
    # have to import anything.
    get_rdata_class(dns.rdataclass.CH, dns.rdatatype.A, False)
    if disable_dynamic_load:
        # Now disable dynamic loading so any subsequent unknown types immediately becomes
...
def from_text(
    ...
    implements the specified rdata class and types.  If there is no
    class-and-types-specific implementation, the GenericRdata class
    is used.
    ...
    with dns.exception.ExceptionWrapper(dns.exception.SyntaxError):
        ...
        if cls != GenericRdata:
            ...
            if token.is_identifier() and token.value == r"\#":
                #
                # Known types using the generic syntax.  Extract the
...
class RdatatypeExists(dns.exception.DNSException):
    ...
    fmt = (
        "The rdata types with class {rdclass:d} and rdtype {rdtype:d} "
        + "already exists."
    )
...
def register_type(
    ...
    *implementation*, a module implementing the types in the usual dnspython
    way.
    ...
    *is_singleton*, a ``bool``, indicating if the types is a singleton (i.e.
    RRsets of the types can have only one member.)

    *rdclass*, the rdataclass of the types, or ``dns.rdataclass.ANY`` if
