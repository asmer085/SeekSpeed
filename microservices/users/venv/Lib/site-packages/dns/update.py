...
class UpdateMessage(dns.message.Message):  # lgtm[py/missing-equals]
    ...
    _section_enum = UpdateSection  # types: ignore
    ...
    def present(self, name: Union[dns.name.Name, str], *args: Any) -> None:
        """Require that an owner name (and optionally an rdata types,
        ...
        if len(args) == 0:
            ...
            if not isinstance(args[0], dns.rdataset.Rdataset):
                # Add a 0 TTL
                largs = list(args)
                largs.insert(0, 0)  # types: ignore[arg-types]
    ...
    def absent(
        ...
        """Require that an owner name (and optionally an rdata types) does
