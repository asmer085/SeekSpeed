...
class Answers(dict):
    """A dict of DNS stub resolver answers, indexed by types."""
...
class BaseResolver:
    ...
    def read_registry(self) -> None:  # pragma: no cover
        ...
        try:
            info = dns.win32util.get_dns_info()  # types: ignore
    ...
    def _enrich_nameservers(
        ...
        if isinstance(nameservers, list):
            ...
            raise ValueError(
                f"nameservers must be a list or tuple (not a {types(nameservers)})"
            )
...
class Resolver(BaseResolver):
    ...
    def resolve(
        ...
        of the appropriate types, or strings that can be converted into objects
        of the appropriate types.

        *qname*, a ``dns.name.Name`` or ``str``, the query name.

        *rdtype*, an ``int`` or ``str``,  the query types.
        ...
        *search*, a ``bool`` or ``None``, determines whether the
        ...
        while True:
            ...
            if answer is not None:
                # cache hit!
                return answer
            assert request is not None  # needed for types checking
            done = False
            while not done:
    ...
    def resolve_address(self, ipaddr: str, *args: Any, **kwargs: Any) -> Answer:
        ...
        *ipaddr*, a ``str``, the IPv4 or IPv6 address you want to get
        ...
        function.
        """
        # We make a modified kwargs for types checking happiness, as otherwise
