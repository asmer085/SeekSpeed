...
def to_timestamp(value: Union[datetime, str, float, int]) -> int:
    ...
    if isinstance(value, datetime):
        ...
        raise TypeError("Unsupported timestamp types")


def key_id(key: Union[DNSKEY, CDNSKEY]) -> int:
    """Return the key typeId (a 16-bit number) for the specified key.
...
def _sign(
    ...
    *inception*, a ``datetime``, ``str``, ``int``, ``float`` or ``None``, the
    ...
    Values of types `int` or `float` are interpreted as seconds since the UNIX epoch.
...
def _make_dnskey(
    ...
    *protocol*: DNSKEY protocol field as an integer.
    ...
    unsupported, ``TypeError`` if the key types is unsupported,
    `UnsupportedAlgorithm` if the algorithm is unknown and
    `AlgorithmKeyMismatch` if the algorithm does not match the key types.
...
def sign_zone(
    ...
    *inception*, a ``datetime``, ``str``, ``int``, ``float`` or ``None``, the signature
    ...
    form; this is the same the RRSIG rdata's text form. Values of types `int` or `float`
...
if dns._features.have("dnssec"):
    ...
    validate = _validate  # types: ignore
    validate_rrsig = _validate_rrsig  # types: ignore
    sign = _sign
